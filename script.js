...
var safeStorage = (function(){
    ...
    try{
        ...
        return {getItem:function(k){return _s[k]!==undefined?_s[k]:null;},setItem:function(k,v){_s[k]=String(v);},removeItem:function(k){delete _s[k];}};
    }
})();

// Firebase compat — loaded via CDN in index.html
...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    function startBanWatcher(username) {
        if (banWatcherRef) banWatcherRef.off(); // امسح القديم لو موجود
        banWatcherRef = _db.ref('students/' + username + '/banned');
        banWatcherRef.on('value', (snap) => {
            if (snap.val() === true) {
                banWatcherRef.off();
                safeStorage.removeItem('isLoggedIn');
                safeStorage.removeItem('username');
    ...
    async function getStudent(username) {
        try {
            const snap = await _db.ref('students/' + username).once('value');
    ...
    async function getReviewByCode(code) {
        try {
            const snap = await _db.ref('reviews').once('value');
    ...
    async function checkLoginState() {
        ...
        if (urlUser) {
            ...
            if (studentFromUrl && studentFromUrl.videoCode && !studentFromUrl.banned) {
                const now = Date.now();
                _db.ref(`students/${urlUser}/lastSeen`).set(now);
                _db.ref(`loginLogs/${urlUser}`).push({ at: now, name: studentFromUrl.name });
        ...
        if (loggedIn === 'true' && username) {
            ...
            if (student && student.videoCode) {
                ...
                _db.ref(`students/${username}/lastSeen`).set(Date.now());
    ...
    document.getElementById('loginForm').addEventListener('submit', async function (e) {
        ...
        if (student && student.videoCode) {
            ...
            // ── سجّل الدخول ──────────────────────────────────────
            const now = Date.now();
            _db.ref(`students/${username}/lastSeen`).set(now);
            _db.ref(`loginLogs/${username}`).push({ at: now, name: student.name });
            // ─────────────────────────────────────────────────────
            await loadVideoContent(student.videoCode, username, student);
            showMain();
            startBanWatcher(username);
    ...
    async function loadVideoContent(code, username, student) {
        ...
        let chapData = {};
        try {
            const snap = await _db.ref('videos/' + code).once('value');
