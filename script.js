...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    async function loadVideoContent(code, username, student) {
        ...
        // دمج الافتراضي مع المضاف من Firebase
        const defaults = CHAPTER_NAMES[code] || { 1: 'Videos' };
        const allIdxs = new Set([...Object.keys(defaults).map(Number), ...Object.keys(customNames).map(Number)]);
        const chapterNames = {};
        Array.from(allIdxs).sort((a,b) => a-b).forEach(idx => {
            chapterNames[idx] = (customNames[idx] && customNames[idx].trim())
                ? customNames[idx]
                : (defaults[idx] || 'فصل ' + idx);
        });
