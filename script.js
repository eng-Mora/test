...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    async function loadVideoContent(code, username, student) {
        ...
        if (branches) {
            ...
            // ── الفصول الإضافية: تظهر مباشرة كـ tabs بدون branch ──
            if (extraIdxs.length > 0) {
                html += '<div class="extra-chapters-section">';
                html += '<div class="chapters-nav">';
                extraIdxs.forEach((chNum, i) => {
                    html += `<button class="chapter-btn ${i === 0 ? 'active' : ''}" onclick="switchExtraChapter(${chNum})">${chapterNames[chNum]}</button>`;
                });
                html += '</div>';
                extraIdxs.forEach((chNum, i) => {
                    const vids = chapData['ch' + chNum] || {};
                    html += buildChapter(chNum, chapterNames[chNum], vids, code, username, i === 0);
                });
                html += '</div>';
            }
