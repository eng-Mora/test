...
// يتحقق من الجهاز قبل السماح بالدخول
// يرجع: 'ok' | 'blocked' | 'registered'
async function checkAndRegisterDevice(username) {
    ...
    try {
        ...
        if (!snap.exists() || !snap.val()) {
            // أول دخول — سجّل الجهاز
            await set(studentRef, deviceId);
            return 'registered';
        }
        const savedId = snap.val();
        if (savedId === deviceId) return 'ok';
        return 'blocked';
    } catch (e) {
