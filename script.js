...
import { getDatabase, ref, get, push, set, onValue, off, runTransaction } from "https://www.gstatic.com/firebasejs/10.8.0/firebase-database.js";
...
// ── Device ID — جهاز الطالب ──────────────────────────────────
function getOrCreateDeviceId() {
    let id = localStorage.getItem('_tp_device');
    if (!id) {
        id = 'dev_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 10);
        localStorage.setItem('_tp_device', id);
    }
    return id;
}

// يتحقق من الجهاز قبل السماح بالدخول — بشكل ذري (atomic) عشان يمنع
// أكتر من جهاز يسجلوا في نفس اللحظة بالظبط (race condition)
// يرجع: 'ok' (الجهاز ده مسموح، سواء كان أول تسجيل أو مطابق) | 'blocked' (جهاز تاني مسجل)
async function checkAndRegisterDevice(username) {
    const deviceId = getOrCreateDeviceId();
    const studentRef = ref(db, `students/${username}/deviceId`);
    try {
        const result = await runTransaction(studentRef, (currentValue) => {
            if (!currentValue) {
                // فاضي — سجّل الجهاز ده (أول واحد يوصل يكسب)
                return deviceId;
            }
            if (currentValue === deviceId) {
                // نفس الجهاز المسجل — سيبه زي ما هو
                return currentValue;
            }
            // جهاز مسجل مختلف — ابطل العملية (يرجع نفس القيمة بدون تغيير)
            return undefined; // abort transaction (يسيب القيمة زي ما هي)
        });

        if (!result.committed) {
            // العملية اتلغت يدوياً (يعني فيه جهاز تاني مسجل بالفعل)
            return 'blocked';
        }
        const finalValue = result.snapshot.val();
        if (finalValue === deviceId) return 'ok';
        return 'blocked';
    } catch (e) {
        console.warn('device check error:', e);
        return 'ok'; // في حالة خطأ شبكة نسمح بالدخول عشان ميتعاقبش الطالب ظلم
    }
}
...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    async function checkLoginState() {
        ...
        if (urlUser) {
            ...
            if (studentFromUrl && studentFromUrl.videoCode && !studentFromUrl.banned) {
                // ── فحص الجهاز ────────────────────────────────────
                const deviceResult = await checkAndRegisterDevice(urlUser);
    ...
    document.getElementById('loginForm').addEventListener('submit', async function (e) {
        ...
        if (student && student.videoCode) {
            ...
            // ── فحص الجهاز ──────────────────────────────────────
            errEl.style.color = '#888';
            errEl.textContent = '⏳ جاري التحقق من الجهاز...';
            const deviceResult = await checkAndRegisterDevice(username);
            if (deviceResult === 'blocked') {
                errEl.style.color = '#dc3545';
