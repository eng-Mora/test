...
function makeVideoEmbed(url) {
    ...
    if (fileId) {
        ...
        // الـ outer div هو اللي بيتشاف — بارتفاع 16:9 صح
        // الـ inner div أكبر عشان يعوض الـ Drive header/footer (~90px فوق و~50px تحت)
        return `<div style="width:100%;aspect-ratio:16/9;overflow:hidden;border-radius:10px;margin-bottom:12px;background:#000;position:relative;">
            <div style="position:absolute;top:-90px;left:0;right:0;bottom:-50px;">
                <iframe src="${embedUrl}" style="width:100%;height:100%;border:none;" frameborder="0" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen webkitallowfullscreen mozallowfullscreen playsinline webkit-playsinline></iframe>
            </div>
        </div>`;
    }
    return `<div style="position:relative;width:100%;padding-bottom:56.25%;height:0;overflow:hidden;border-radius:10px;margin-bottom:12px;">
        <iframe src="${url}" style="position:absolute;top:0;left:0;width:100%;height:100%;border:none;border-radius:10px;" frameborder="0" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen playsinline></iframe>
    </div>`;
}
